print("Enter the first number: ") #enter first number
num1 = int(input()) #num1 is a variable
print("Enter the second number: ")
num2 = int(input())
num1EqualNum2 = num1 == num2 #shows whether two numbers are same
print("The numbers are equal: " + str(num1EqualNum2))
num1GreaterThanNum2 = num1 > num2
print("The first number is greater than the second: " + str(num1GreaterThanNum2))
num1LessThanNum2 = num1 < num2
print("The first number is less than the second: " + str(num1LessThanNum2))

print(f'is num1 greater than num 2? \n{num1 > num2}')
# print(f'''is num1 less than num 2? 
# {num1 < num2}''')
# print(f'''is num1 equal to num 2? 
# {num1 == num2}''')

# print(type(num1))

# # if type(num1) == 'int': #OR
# if type(num1) == int:
#     print(f'{num1} is a number')